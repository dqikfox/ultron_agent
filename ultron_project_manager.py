...
class UltronProjectManager:
    ...
    def __init__(self):
        ...
        # Security: Load Bedrock credentials from environment variable
        # Set via: $env:AWS_BEDROCK_API_KEY = "your-key" or ~/.aws/credentials
        self.bedrock_api_key = os.getenv("AWS_BEDROCK_API_KEY", None)
        if not self.bedrock_api_key:
            log_error("project_manager", "AWS_BEDROCK_API_KEY not set. Bedrock fallback disabled.")
...
if __name__ == "__main__":
    ...
    print("=== ULTRON PROJECT MANAGER AI === - ultron_project_manager.py:402")
    print(f"Status: {result['status']} - ultron_project_manager.py:403")
    print(f"Actions Taken: {result['actions_taken']} - ultron_project_manager.py:404")
    print(f"Health Score: {report['health_score']}/100 - ultron_project_manager.py:405")
    print(f"Report saved to: logs/project_management_report.json - ultron_project_manager.py:406")
