...
### Core Architecture
- **Multi-Service Architecture**: ULTRON runs as multiple concurrent services (AI Chat, Web GUI, API Server)
- **Event-Driven Communication**: Uses `utils/event_system.py` for cross-component messaging
- **Modular Tool System**: Tools in `tools/` directory with `match()` and `execute()` methods
- **Fallback Chains**: Voice (ElevenLabs → pyttsx3 → Console), AI models (Ollama → OpenAI → Local)

### Primary Entry Points
- **`main.py`**: Main application entry point with signal handling and async initialization
- **`run.bat`**: Production launcher starting all services (ports 8000, 8080, 5000)
- **`nvidia_enhanced_ultron.py`**: AI chat server with NVIDIA integration
- **`web_gui_server.py`**: Web interface server
- **`api_server.py`**: REST API server with FastAPI

### Key Components
- **`brain.py`**: Core AI reasoning with Ollama integration, caching, and tool orchestration
- **`voice.py`**: Multi-engine voice system with ElevenLabs TTS/STT and fallbacks
- **`ollama_manager.py`**: Model management and switching for Ollama
- **`config.py`**: Configuration management (currently stub - use `ultron_config.json`)
- **`tools/`**: Plugin architecture with 15+ specialized tools (file ops, web search, code execution, etc.)
- **`utils/event_system.py`**: Async event system for component communication
- **`utils/performance_monitor.py`**: System monitoring and metrics collection
...
# Individual services
python nvidia_enhanced_ultron.py    # AI Chat (port 8000)
python web_gui_server.py          # Web GUI (port 8080)
python api_server.py              # API Server (port 5000)
...
- **VS Code**: Use `F5` with `debugpy` configurations in `.vscode/launch.json`
- **Performance**: Monitor via `utils/performance_monitor.py`
- **Events**: Debug via `utils/event_system.py` event history
