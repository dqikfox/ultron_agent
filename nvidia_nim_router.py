...
class UltronNvidiaRouter:
    ...
    def __init__(self, api_key: str = None, config: Dict[str, Any] = None):
        ...
        self.response_cache = {}
        self.cache_max_size = 100  # Maximum cached responses
        ...
        logger.info(f"NVIDIA NIM Router initialized with {len(self.models)} models and caching enabled")

    def _generate_cache_key(self, prompt: str, model: str, **kwargs) -> str:
        """Generate a unique cache key for the request"""
        # Create a hash of the prompt and parameters
        key_data = f"{model}:{prompt}:{kwargs.get('max_tokens', 'default')}:{kwargs.get('temperature', 0.7)}"
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_hit_ratio": f"{(valid_entries / max(total_entries, 1)) * 100:.1f}%"
        }
