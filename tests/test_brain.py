...
async def test_plan_and_act_with_tool_call(brain):
    ...
    with (
        patch.object(brain, "query_llm", new_callable=AsyncMock) as mock_query_llm,
        patch.object(
            brain.tools[0], "execute", return_value="Tool executed"
        ) as mock_execute,
    ):
...
async def test_plan_and_act_direct_tool_match(brain):
    ...
    with (
        patch.object(brain.tools[0], "match", return_value=True),
        patch.object(
            brain.tools[0], "execute", return_value="Direct execution"
        ) as mock_execute,
    ):
