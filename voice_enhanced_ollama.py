...
class UltronEnhancedVoiceManager:
        ...
        def self_test(self) -> dict:
            """
            Run diagnostics for TTS and voice recognition subsystems.
            Returns a dict with status and details.
            """
            result = {"status": "ok", "errors": [], "details": {}}
            # Test TTS
            try:
                self.speak("Voice diagnostics test.")
                result["details"]["tts"] = "ok"
            except Exception as e:
                result["status"] = "fail"
                result["errors"].append(f"TTS error: {e}")
            # Test recognizer/microphone
            try:
                if self.recognizer and self.microphone:
                    result["details"]["recognizer"] = "ok"
                else:
                    raise Exception("Recognizer or microphone not initialized")
            except Exception as e:
                result["status"] = "fail"
                result["errors"].append(f"Recognizer error: {e}")
            return result
            ...
            if self.agent_core:
                try:
                    ...
                    if hasattr(self.agent_core, 'process_command'):
                        if inspect.iscoroutinefunction(self.agent_core.process_command):
                            # Run async method in event loop
                            try:
                                loop = asyncio.get_running_loop()
                                task = loop.create_task(self.agent_core.process_command(command))
                                response = None  # Will be handled by task completion
                                self.voice_manager.speak("Processing your command...")
                            except RuntimeError:
                                # No running loop, create new one
                                response = asyncio.run(self.agent_core.process_command(command))
                        ...
                        self.voice_manager.speak("Command processing not available.")
                except Exception as e:
