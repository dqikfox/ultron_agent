...
class AgentGUI:
    ...
    def _run_async_voice_listen(self):
        """Helper to run async voice listen in a thread"""
        def voice_thread():
            try:
                loop = asyncio.new_event_loop()
                asyncio.set_event_loop(loop)
                text = loop.run_until_complete(self.agent.voice.listen())
                loop.close()
                
                if text:
                    self.root.after(0, lambda: self._handle_voice_result(text))
                else:
                    self.root.after(0, lambda: messagebox.showinfo("Voice", "No speech detected."))
            except Exception as e:
                self.root.after(0, lambda: messagebox.showerror("Voice", f"Voice input error: {e}"))
            finally:
