...
Deno.serve(async (req) => {
    ...
    bucketConfig.allowed_mime_types = ["image/*", "video/*", "audio/*", "application/pdf", "text/*"];
    ...
    const policyQueries = [
        ...
        `CREATE POLICY "Public Access for file-uploads" ON storage.objects FOR SELECT USING (bucket_id = 'file-uploads');`,
        // Allow public to insert objects in this bucket
        `CREATE POLICY "Public Upload for file-uploads" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'file-uploads');`,
        // Allow public to update objects in this bucket
        `CREATE POLICY "Public Update for file-uploads" ON storage.objects FOR UPDATE USING (bucket_id = 'file-uploads');`,
        // Allow public to delete objects in this bucket
        `CREATE POLICY "Public Delete for file-uploads" ON storage.objects FOR DELETE USING (bucket_id = 'file-uploads');`
    ];
